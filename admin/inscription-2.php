...
if(isset($_GET['edit'])){
  ...
  if (!empty($_POST)) {
    ...
    if($civilite == 'civil'){$errors['civilite'] = 'Le choix de la civilité est obligatoire';}elseif($civilite == $civilite1){$errors1['civilite'] = 'Aucune modification enregistré';} // 1
    else{$column ['civilite'] = $civilite; }
    if($status == 'stat'){$errors['status'] = 'Le choix du statuts est obligatoire';}elseif($status == $status1){$errors1['status'] = 'Aucune modification enregistré';}     // 2
    ...
    else{$column ['nom'] = $nom ;}            // 3
    if($prenom == ''){$errors['prenom'] = 'La saisie du prenom est obligatoire';}elseif($prenom == $prenom1){$errors1['prenom'] = 'Aucune modification enregistré';}    // 4
    ...
    if($pseudo == ''){$errors['pseudo'] = 'La saisie d\'un pseudo est obligatoire';}elseif($pseudo == $pseudo1){$errors1['pseudo'] = 'Aucune modification enregistré';}    // 5
    ...
    if($email == ''){$errors['email'] = 'La saisie de l\'adresse mail est obligatoire';}elseif(!filter_var($email, FILTER_VALIDATE_EMAIL)){
      $errors[] = '1/ - Le format de l\'email n\'est pas valide';}elseif($email == $email1){$errors1['email'] = 'Aucune modification enregistré';      // 6
    }elseif($email != $email1 && $email == $email2){$errors['email'] = 'L\'email existe déjà dans la base';}
    else{$column ['email'] = $email; }
    if($telephone == ''){$errors['telephone'] = 'La saisie d\'un numéro de télephone est obligatoire';}elseif($telephone == $telephone1){$errors1['telephone'] = 'Aucune modification enregistré';}//7
...
<table class="table table-striped table-condensed">
  <tr class="text-center">
    ...
    <th class="text-center">Date d'enrégistrement</th>
